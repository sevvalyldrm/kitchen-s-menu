...
btnList.forEach((value) => {
  ...
  button.setAttribute("btn-container", value);
...
function buttonClick() {
  const result = (this.getAttribute("btn-container") == "All") ? menu : menu.filter((item) => {
    return item.category == this.getAttribute("btn-container");
  });
  addMenu(result);
};
  ...
  for (let item in showMenu) {
    ...
    <div class="menu-info">
      <div class="menu-title">
        <h1>${showMenu[item].title}</h1>
        <h1 class="price">${showMenu[item].price}</h1>
      </div>
      <div class="menu-text">
      ${showMenu[item].desc}
      </div>
